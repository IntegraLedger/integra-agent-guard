 ...
 *
 * The parser is in the round-trip deliberately (audit DF-6). A round-trip that stops at the verify step
 * cannot catch a placement and a parser that disagree about which fields the wire carries — which is
 * exactly how the plan's two halves originally failed to compose: B3 placed one field while B4's schema
 * required two. The placement is consumed here as a PUBLISHED version from the private registry
...
describe("ACP Phase-B exit round-trip", () => {
  it("carries a real atrHash from an assembled ATR out through the buyer's parser", async () => {
    ...
    expect(metadata["merchant_order_ref"]).toBe("SO-4417");
