 ...
 *
 *   pnpm mutation agent-guard   # a single package
 ...
 * are the same code's.
 *
 * agent-guard rose 94 -> 96 when the universal parser landed. The rise is the new file's own doing: at
 ...
 *
 * agent-guard's earlier entry, SUPERSEDED by the universal parser's rise to 96 (the header paragraph above): it stayed
...
if (!pkg) {
  throw new Error(
    "STRYKER_PKG is required — run `pnpm mutation <package>` (e.g. `pnpm mutation agent-guard`).",
  );
}
